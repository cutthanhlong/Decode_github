...
class DayPickerViewPager extends ViewPager {
    ...
    public DayPickerViewPager(Context context, AttributeSet attrs) {
        super(context, attrs);
        TOUCH_SLOP_SQUARED = ViewConfiguration.get(context).getScaledTouchSlop() * ViewConfiguration.get(context).getScaledTouchSlop();
    ...
    private class CheckForLongPress implements Runnable {
        ...
        public void run() {
            if (mDayPickerPagerAdapter != null) {
                ...
                mTempSelectedDate = mDayPickerPagerAdapter.resolveStartDateForRange((int) mInitialDownX, (int) mInitialDownY, getCurrentItem());
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (mIsLongPressed && ev.getAction() == MotionEvent.ACTION_UP || ev.getAction() == MotionEvent.ACTION_CANCEL) {
            if (Config.DEBUG) {
            ...
            if (ev.getAction() == MotionEvent.ACTION_UP) {
                if (mDayPickerPagerAdapter != null) {
                    mTempSelectedDate = mDayPickerPagerAdapter.resolveEndDateForRange((int) ev.getX(), (int) ev.getY(), getCurrentItem(), false);
            ...
            if (mScrollingDirection == NOT_SCROLLING) {
                if (mDayPickerPagerAdapter != null) {
                    mTempSelectedDate = mDayPickerPagerAdapter.resolveEndDateForRange((int) ev.getX(), (int) ev.getY(), getCurrentItem(), true);
